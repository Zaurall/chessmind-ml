...
class RLAgent:
    ...
    def get_q_value(self, state, action):
        return next(
            (q_value for q_state, q_action, q_value in self.q_values if q_state == state and q_action == action), 0.0)
    ...
    def choose_action(self, state):
        return random.choice(
            list(chess.Board(state).legal_moves)) if random.random() < self.epsilon else self.get_best_action(
            chess.Board(state))
